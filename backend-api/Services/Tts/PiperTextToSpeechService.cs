...
/// to shell out to — no bindings/native interop needed. Piper emits WAV, which
/// is what <see cref="ITextToSpeechService"/> requires.
/// </summary>
public class PiperTextToSpeechService(IOptions<TtsOptions> options) : ITextToSpeechService
{
    private readonly PiperOptions _options = options.Value.Piper;
    ...
    public async Task<SynthesizedSpeech> SynthesizeAsync(string text, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(_options.VoiceModelPath))
        {
            throw new InvalidOperationException(
                "Tts:Piper:VoiceModelPath is not configured. Download a Piper voice model " +
        ...
        try
        {
            ...
            return new SynthesizedSpeech(wav, SpeechAudioFormat.Wav);
        }
        finally
        {
