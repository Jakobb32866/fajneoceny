...
export function GradeSheet({ subjectId }: { subjectId: string }) {
  ...
  // list renders — so the subject list has to be invalidated alongside. The
  // dashboard's cross-subject "last grades" digest is invalidated too.
  //
  // refetch() (not invalidate) for this sheet's own data: adding the *first*
  // component turns the endpoint from 404 into 200, and a 404 leaves no cache
  // entry for invalidate() to mark stale — so we force the fetch instead.
      ...
      <Card elevated style={{ margin: theme.spacing[4], gap: theme.spacing[3] }}>
        ...
        <View style={{ flexDirection: 'row', gap: theme.spacing[6] }}>
          ...
          <View>
            ...
            <Text.Mono style={{ fontSize: theme.font.size.title }}>
              {grades.totalWeightPercent.toFixed(0)}%
            </Text.Mono>
          </View>
        </View>
      </Card>
        ...
        {grades.components.map((component) => (
          <Card key={component.id}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              ...
              <Badge
                label={formatPercent(component.averageScorePercent)}
                variant={gradeBadgeVariant(component.averageScorePercent)}
              />
            </View>
              ...
              <View style={{ marginTop: theme.spacing[3] }}>
                {component.entries.map((entry, index) => (
                  <View key={entry.id}>
                    ...
                    >
                      <Text.BodySm style={{ flex: 1, color: theme.colors.text.secondary }}>
                        {entry.name}
                      </Text.BodySm>
                      <Text.Mono
                        style={{ color: theme.colors.text.secondary, marginRight: theme.spacing[3] }}
                      >
