...
export function useCachedQuery<T>(key: string, fetcher: () => Promise<T>, options: Options = {}): Result<T> {
  ...
  // Forces a fresh fetch immediately. Unlike invalidate(), this also works
  // when there is no cache entry yet — e.g. after the first fetch rejected
  // (a 404 for a subject with no grading scheme) and so nothing was cached to
  // mark stale. Used after mutations that must reflect on the current screen.
